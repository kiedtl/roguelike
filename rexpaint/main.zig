...
pub const RexMap = struct {
    ...
    pub fn initFromFile(alloc: std.mem.Allocator, filename: [*c]const u8) !Self {
        var self: RexMap = undefined;
        self.alloc = alloc;

        var filestream = zlib.gzopen(filename, "rb");
        if (filestream == null) {
        ...
        // TODO: handle error
        _ = zlib.gzseek(filestream, 16, zlib.SEEK_SET);
        ...
        while (z < self.layers) : (z += 1) {
            ...
            while (x < self.width) : (x += 1) {
                ...
                while (y < self.height) : (y += 1) {
                    ...
                    //_ = zlib.gzread(filestream, &tile, 10); // TODO: check ret val
            ...
            // TODO: handle error
            _ = zlib.gzseek(filestream, @intCast(c_long, new_offset), zlib.SEEK_SET);
        }
...
pub fn main() anyerror!void {
    ...
    while (y < map.height) : (y += 1) {
        ...
        while (x < map.width) : (x += 1) {
            ...
            const utf8_len = try std.unicode.utf8Encode(tilemap[tile.ch], &utf8_out);
            if (utf8_len > 0) {
