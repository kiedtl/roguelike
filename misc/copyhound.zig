//! Stolen from TigerBeetle. For debugging purposes; not distributed with the
//! game.
//!
//! (c) Tigerbeetle authors and contributors.
//!
...
pub fn main() !void {
    ...
    const line_buffer = try allocator.alloc(u8, 162 * 1000 * 1000);
    ...
    while (try in_stream.readUntilDelimiterOrEof(line_buffer, '\n')) |line| {
        ...
        if (current_function) |function| {
            ...
            if (cut(line, "@llvm.memcpy")) |c| {
                const size = extract_memcpy_size(c.suffix) orelse {
...
fn extract_function_name(define: []const u8, buf: []u8) ?[]const u8 {
    ...
    const mangled_name = (cut(define, "@") orelse return null).suffix;
...
fn extract_memcpy_size(memcpy_call: []const u8) ?u32 {
    const call_args = (cut(memcpy_call, "(") orelse return null).suffix;
    var level: u32 = 0;
    var arg_count: u32 = 0;
    ...
    const size_arg = (cut(args_after_size, ",") orelse return null).prefix;

    const size_value = (cut(size_arg, " ") orelse return null).suffix;
