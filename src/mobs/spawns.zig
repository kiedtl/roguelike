...
pub fn chooseMob(which_table: SpawnTable, z: usize, maybe_class: ?[]const u8) !*const mobs.MobTemplate {
    ...
    if (maybe_class) |class| {
        table = @TypeOf(table).init(state.alloc);
    ...
    inline for (spawn_table_files) |sptable_file| {
        ...
        for (sptable_file.sptable, 0..) |*table, i| {
            ...
            for (spawndatas.items) |spawndata| {
                table.append(.{
                    .id = utils.cloneStr(spawndata.id, state.alloc) catch err.oom(),
                    .classtype = utils.cloneStr(spawndata.classtype, state.alloc) catch err.oom(),
