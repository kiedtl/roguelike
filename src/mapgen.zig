...
fn _add_player(coord: Coord, alloc: mem.Allocator) void {
    ...
    state.player.inventory.pack.append(Item{ .Consumable = &items.EmberlingTrapKit }) catch err.wat();
    state.player.inventory.pack.append(Item{ .Consumable = &items.SparklingTrapKit }) catch err.wat();
}
